...
class HierarchicalConformerEncoder(NeuralModule, Exportable):
    ...
    def __init__(
        ...
        gating_method= 'Sigmoid', # FiLM, Sigmoid or None 
        ...
        self.message_passer_fn = torch.nn.Linear(d_model, d_model)
        self.relu_message_passer = torch.nn.ReLU()
        self.message_passer = lambda x: self.relu_message_passer(self.message_passer_fn(x))
    ...
    def forward_for_export(self, audio_signal, length):
        ...
        for repeat in range(len(self.CrossConformerLayers)):
            ...
            context_info = self.message_passer(audio_signal)

            if self.downsampling_type == 'pooling': 
                downsampled_audio_signal = self.downsampling(context_info.transpose(1, 2)).transpose(1, 2) # transpose so that the sequence length is the first dimension
                downsampled_length = length.div(3, rounding_mode=None).ceil().int()

            elif self.downsampling_type == 'conv':
                downsampled_audio_signal, downsampled_length = self.downsampling(context_info, length)
            ...
            conjoined_downsampled_sequence = conjoined_downsampled_sequence[non_padding_idxs] # removes padding from the sequence
            ...
            # here we create context sequences from every sequence in the batch
            ...
            if repeat == len(self.CrossConformerLayers) - 1 and self.checkpoint_last_layer == False:
