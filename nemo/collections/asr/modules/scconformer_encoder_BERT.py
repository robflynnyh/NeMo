...
from nemo.collections.asr.parts.submodules.CTCBert import BertEncoderCTC
...
class SelfConditionedConformerEncoder(NeuralModule, Exportable):
    ...
    def __init__(
        ...
        if exists(ROBERTA_LOAD_PATH):
            self.BERT.load_roberta(ROBERTA_LOAD_PATH)
    
        self.BERT.freeze_BERT(freeze = freeze_roberta)
        print('ROBERTA IS FROZEN') if freeze_roberta else print('ROBERTA IS NOT FROZEN')

        assert self.self_condition, "You probably set this to False by mistake!"
        ...
        for lth, layer in enumerate(self.layers):
            ...
            if lth != len(self.layers) - 1:
                ...
                if self.self_condition == True:
                    BERT_out, BERT_residual = self.BERT(
                        x = iterim_post,
                        residual = BERT_residual,
                        layer_num = lth,
                        attention_mask = att_mask,
                        pos_emb = pos_emb,
                    )
                    audio_signal = audio_signal + BERT_out
